...
latinBreaks = r"[.,!?;:]"
splitReg = re.compile(
    "{arabic}|{chinese}|{latin}".format(
        arabic=arabicBreaks, chinese=chineseBreaks, latin=latinBreaks
    )
)
...
class Translator:
    ...
    def __init__(
        ...
        **kwargs
        ...
        logging.debug(
            "Instant translate: lang_from=%s, lang_to=%s, lang_swap=%s"
            % (lang_from, lang_to, lang_swap)
        )
        if lang_from != "auto" and lang_swap is not None:
            raise RuntimeError(
                "Unexpected arguments: langFrom={}, langTo={}, langSwap={}, {}".format(
                    text, lang_from, lang_to, lang_swap
                )
            )
    ...
    def run(self):
        ...
        for chunk in splitChunks(self.text, self.chunksize):
            ...
            try:
                ...
                self.lang_detected = langConversionDic.get(
                    self.lang_detected, self.lang_detected
                )
                logging.info(
                    "firstChunk=%s, lang_from=%s, lang_detected=%s, lang_to=%s, lang_swap=%s"
                    % (
                        self.firstChunk,
                        self.lang_from,
                        self.lang_detected,
            ...
            self.translation += "".join(
                sentence["trans"] for sentence in response["sentences"]
            )
